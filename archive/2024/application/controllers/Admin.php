...
class Admin extends CI_Controller {
    ...
    public function makeResult() {
        ...
        //Subject Load End
        //Max Mark Load Start
        $maxData = $this->General_model->queryData("SELECT `year`, class_id, exam, sub_code, type, MAX(mark) as mark, full_mark FROM  mark WHERE `year`=? GROUP BY `year`, class_id, exam, sub_code, type", array(date('Y')));
        $max = array();
        ...
        //Max Mark Load End
        //Mark Load Start
        $markData = $this->General_model->selectData('mark', array('year' => date('Y')));
        $mark = array();
    ...
    public function markuploadsingle($id = '') {
        ...
        $data = array(
            ...
            'mark' => $this->General_model->selectData('mark', array('student_id' => $row['id'], 'year' => date('Y'), 'class_id' => $row['class']))
        );
    ...
    public function markUpload() {
        ...
        if ($this->input->post('mark')) {
            if ($this->input->post()) {
                ...
                $data['mark'] = $this->General_model->queryData("SELECT * FROM `mark` WHERE `class_id`=? AND `year`=? AND `exam`=? AND `sub_code`=? AND `student_id` IN (SELECT `id` FROM `students` WHERE `class`=? AND `section`=?) ", array($post['class'], date('Y'), $post['exam'], $data['subject']['code'], $post['class'], $post['section']));
    ...
    public function setting() {
        ...
        if ($this->input->post('admissiontime')) {
            ...
            $this->session->set_flashdata('updateMesg', '<div class="alert btn-primary"><a href="javascript:avoid(0)" class="close" data-dismiss="alert">&times;</a>Update Successfully.</div> ');
        ...
        if ($this->input->post('change_password')) {
            ...
            if ($password == $cpassword) {
                $updateData['password'] = md5('M1' . $password . 'z$');
                $this->General_model->updateData('users', $updateData, array('id' => 2));
                $this->session->set_flashdata('updateMesg1', '<div class="alert btn-primary"><a href="javascript:avoid(0)" class="close" data-dismiss="alert">&times;</a>সফল ভাবে পাসওয়ার্ড পরিবর্তন হয়েছে।</div> ');
                redirect('admin/setting');
            } else {
                $this->session->set_flashdata('errorMesg', '<div class="alert btn-danger"><a href="javascript:avoid(0)" class="close" data-dismiss="alert">&times;</a>পাসওয়ার্ডটি ভুল দিয়েছেন।</div>');
                redirect('admin/setting');
            }
        }
        
// Result Publish START [NIRZOR]

        $query = $this->db->get('publish');
        
        if ($this->input->post('result_publish')) {
            $result_password = $this->input->post('result_password');
            if(isset($result_password)){
                foreach ($query->result() as $row){
                    $publish_pass= $row->publish_pass;
                    if ($publish_pass == md5($result_password)) {
                        $updateData['status'] = 1;
                        $this->General_model->updateData('publish', $updateData, array('id' => 1));
                    
                        $this->session->set_flashdata('updateMesgPublish', '<div class="alert btn-primary"><a href="javascript:avoid(0)" class="close" data-dismiss="alert">&times;</a>সফল ভাবে রেজাল্ট পাবলিশ হয়েছে।</div> ');
                        redirect('admin/setting');
                        
                        
                    } else {
                        $this->session->set_flashdata('errorMesgPublish', '<div class="alert btn-danger"><a href="javascript:avoid(0)" class="close" data-dismiss="alert">&times;</a> পাসওয়ার্ডটি ভুল দিয়েছেন।</div>');
                        }
                    redirect('admin/setting');
                }
            }
        ...
        if ($this->input->post('result_publish_off')) {
            $publish_off=$this->input->post('publish_off');
            if(isset($publish_off)){
                foreach ($query->result() as $row){
                    $publish_off_pass= $row->publish_off_pass;
                    if ($publish_off_pass == md5($publish_off)) {
                        $updateData['status'] = 0;
    	                    $this->General_model->updateData('publish', $updateData, array('id' => 1));
                    
                        $this->session->set_flashdata('updateMesgPublishOff', '<div class="alert btn-primary"><a href="javascript:avoid(0)" class="close" data-dismiss="alert">&times;</a>সফল ভাবে রেজাল্ট পাবলিশ বন্ধ হয়েছে।</div> ');
                        redirect('admin/setting');
                        
                        
                    } else {
                        $this->session->set_flashdata('errorMesgPublishOff', '<div class="alert btn-danger"><a href="javascript:avoid(0)" class="close" data-dismiss="alert">&times;</a> পাসওয়ার্ডটি ভুল দিয়েছেন।</div>');
                        }
                    redirect('admin/setting');
                }
            }
            ...
            // Notice
        if ($this->input->post('result_off_notice')) {
            $off_notice=$this->input->post('off_notice');
            if(isset($off_notice)){
                foreach ($query->result() as $row){
                    $notice= $row->result_off_notice;
                    $updateData['result_off_notice'] = $off_notice;
                    $this->General_model->updateData('publish', $updateData, array('id' => 1));
                    
                    $this->session->set_flashdata('updateMesgNotic', '<div class="alert btn-primary"><a href="javascript:avoid(0)" class="close" data-dismiss="alert">&times;</a>সফলভাবে নোটিশ পাবলিশ হয়েছে।</div> ');
                        redirect('admin/setting');
                }
            }
